...
void skimTreeJERBase(string outputStr,string st){
	...
	TH1D * th1[102];
	for(int i=0;i<102;i++){
		th1[i]=new TH1D(Form("totalMass_%d",i),Form("totalMass_%db",i),250,0,5000);
		th1[i]->Sumw2();
	}
	
	TH1D * th2[5];
	for(int i=0;i<5;i++){
		th2[i]=new TH1D(Form("totalMassScale_%d",i),Form("totalMassScale_%db",i),250,0,5000);
		th2[i]->Sumw2();
	}
	...
	for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
		...
		for (int i=0;i<101;i++)th1[i+1]->Fill(mjjred,pdfscaleSysWeights[i+9]/pdfscaleSysWeights[9]);
		th1[0]->Fill(mjjred);
		
		th2[0]->Fill(mjjred,pdfscaleSysWeights[0]);
		th2[1]->Fill(mjjred,pdfscaleSysWeights[1]);
		th2[2]->Fill(mjjred,pdfscaleSysWeights[2]);
		th2[3]->Fill(mjjred,pdfscaleSysWeights[3]);
		th2[4]->Fill(mjjred,pdfscaleSysWeights[6]);
	...
	TFile* output=new TFile(Form("pdfHisto/%s.root",outputStr.data()),"recreate");
	TH1D* thf=new TH1D("cut","cut",11,-0.5,11.5);
	for(int i=0;i<11;i++)thf->SetBinContent(i+1,nPass[i]);
	thf->Write();
	for(int i=0;i<102;i++)th1[i]->Write();
	for(int i=0;i<5;i++)th2[i]->Write();
	output->Close();
	
	for(int i=0;i<102;i++)delete th1[i];
	for(int i=0;i<5;i++)delete th2[i];
}
