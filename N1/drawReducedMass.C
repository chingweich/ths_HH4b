...
// gStyle->SetPadLeftMargin(1.559);
 ...
 TH1D * th1[2];
 th1[0]=(TH1D*) tf2[2]->Get("totalMass_bb");
 for(int i=0;i<3;i++){
	 TH1D * th2=(TH1D*) tf2[2]->Get(Form("totalMass_%s",hadflv[i+1].data()));
	 th2->Sumw2();
	 th1[0]->Add(th2);
 }
 th1[1]=(TH1D*) tf2[2]->Get("totalMassRed_bb");
 for(int i=0;i<3;i++){
	 TH1D * th2=(TH1D*) tf2[2]->Get(Form("totalMassRed_%s",hadflv[i+1].data()));
	 th2->Sumw2();
	 th1[1]->Add(th2);
 }
 th1[0]->GetXaxis()->SetRangeUser(1900,2800);
