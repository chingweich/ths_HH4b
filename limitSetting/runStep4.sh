...
do
	combineCards.py SR1=outputs/datacards/HH_mX_$i\_HH_LL_13TeV.txt SR2=outputs/datacards/HH_mX_$i\_HH_TT_13TeV.txt AT1=outputs/datacards/HH_mX_$i\_HH_LL_13TeV_fail.txt AT2=outputs/datacards/HH_mX_$i\_HH_TT_13TeV_fail.txt > outputs/datacards/HH_mX_$i\_bump_13TeV.txt
	echo "mass point $i"
	mkdir Limits_data_Summer16/sig$i
...
#        combine -M MaxLikelihoodFit --rMin=-50 --rMax=50 --saveNormalizations --plot --saveShapes --saveWithUncertainties  -v 4 outputs/datacards/HH_mX_$i\_bump_13TeV.txt
	...
	mv higgsCombineTest.Asymptotic.mH$i\.root Limits_data_Summer16/CMS_$i\_HH4b_13TeV_asymptoticCLs.root
