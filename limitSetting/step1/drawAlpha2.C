...
//tf4->Draw("same");
TF1 * tf5[3];
tf5[0]=(TF1*)tf1->Get("ths_fit");
tf5[1]=(TF1*)tf1->Get("ths_ErrUp");
tf5[2]=(TF1*)tf1->Get("ths_ErrDn");
TLegend *leg = new TLegend(0.4, 0.58, 0.3, 0.77);
  
  leg->SetBorderSize(0);
  leg->SetFillColor(0);
  leg->SetFillStyle(0);
  leg->SetTextSize(0.04);
  leg->AddEntry(th2," R_{p/f}","ple");
  th2->SetFillColor(0);
  tf5[0]->SetFillColor(0);
  tf5[1]->SetFillColor(0);
  leg->AddEntry(tf5[0],Form(" Fit: %.2f+%.2ex+%.2ex^{2}",tf5[0]->GetParameter(0),tf5[1]->GetParameter(1),tf5[2]->GetParameter(2)));
  leg->AddEntry(tf5[1]," Fit error");
leg->Draw("same");
for(int i=0;i<3;i++){
	tf5[i]->Draw("same");
	if(i==0)continue;
	tf5[i]->SetLineStyle(2);
}
